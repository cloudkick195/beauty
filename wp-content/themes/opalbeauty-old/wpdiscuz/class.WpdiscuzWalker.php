...
class WpdiscuzWalker extends Walker_Comment implements WpDiscuzConstants {
    ...
    public function start_el(&$output, $comment, $depth = 0, $args = [], $id = 0) {
        ...
        if ($this->options->thread_layouts["showAvatars"] && $this->options->wp["showAvatars"]) {
            ...
            if(get_field('user_avatar', 'user_'. $comment->user_id)){
                $authorAvatar = "<a href=".get_author_posts_url( $comment->user_id)."> <img class='avatar avatar-" . esc_attr($user["gravatarArgs"]["wpdiscuz_gravatar_size"]) . " photo' width='" . esc_attr($user["gravatarArgs"]["wpdiscuz_gravatar_size"]) . "' height='" . esc_attr($user["gravatarArgs"]["wpdiscuz_gravatar_size"]) . "' src='" . get_field("user_avatar", "user_". $comment->user_id) . "' alt='trackback'></a>";
            }
            $showAvatar = true;
        ...
        $replace[] = '<a href="'.get_author_posts_url( $comment->user_id).'">'.$user["authorNameHtml"].'</a>';
        ...
        if ($this->options->thread_layouts["showVotingButtons"] && $isApproved) {
            ...
            $replace[] = '';
            $replace[] = $voteResult.__('likes', 'opalbeauty');
        ...
        if ($this->options->thread_layouts["showCommentDate"]) {
            ...
            $replace[] = "<span class='calenda-icon' aria-hidden='true'></span>";
            $replace[] = get_comment_date($this->options->wp["dateFormat"]);
        ...
        if ($args["layout"] == 1) {
            ...
            $headerComponent = str_replace(["{AUTHOR}", "{STATUS}", "{SHARE}", "{LINK}"], [$args["components"]["author.html"], $showStatus ? $args["components"]["status.html"] : "", $showShare ? $args["components"]["share.html"] : "", $showLink ? $args["components"]["link.html"] : ""], $args["components"]["header.html"]);
            
            $headerComponent .= '<div class="cleared"></div>';
            $footerComponent = '';
            if(	is_singular( 'beauty' )){
                $headerComponent .= 
                '<div class="review-meta">
                    '.get_comment_meta($comment->comment_ID, 'beauty_rating', true).'/5 <img src='.get_theme_path('/assets/images/full-star.svg').'>
                </div>';
                $footerComponent = 
                '<div class="date-span">
                    <span class="calenda-icon"><img src='.get_theme_path('/assets/images/calenda-black.svg').'> '.get_comment_date($this->options->wp["dateFormat"]).'</span>
                </div>';
            }else{
                $footerComponent = 
                '<div class="date-span">
                    <span class="calenda-icon">'. esc_html(opal_dateDiff($comment->comment_date_gmt)).'</span>
                </div>';
            }
            ...
            $rightComponent = str_replace(["{HEADER}", "{REPLY_TO}", "{TEXT}", "{DATE}", "{FOOTER}"], [$headerComponent, $showReplyTo ? $args["components"]["reply_to.html"] : "", $args["components"]["text.html"] . $lastEdited,  $showDate ? $args["components"]["date.html"] : "", $footerComponent], $args["components"]["right.html"]);
        ...
        // $output .= '
        //     <div class="comment-wrap img-des">
        //        <div class="img">'. $avatar.'</div>
        //         <div class="des">
        //             <h5 class="box-title">'.$user["authorNameHtml"].'</h5>
        //             <div class="excerpt">'.$comment->comment_content.'</div>
        //             <div class="date-span"><span class="calenda-icon">'.get_comment_date($this->options->wp["dateFormat"]).'</span></div>
        //         </div>
        //     </div>
        // ';
    }
