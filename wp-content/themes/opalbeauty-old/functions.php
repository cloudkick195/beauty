...
add_action('wp_enqueue_scripts','bangtaihaitan_load_frontend');
function bangtaihaitan_load_frontend(){
    ...
    wp_enqueue_script( 'owl.carousel.min.js' ,get_template_directory_uri() .'/assets/OwlCarousel2-2.3.4/owl.carousel.min.js','',false,true);
...
function header_page($title, $backuUrl = ''){
    echo '<div class="header-page">
        <div class="top-element">
            <a href="#" onclick="GoBackWithRefresh();return false;" class="back-main">
...
function opal_filter_handler( $approved , $commentdata )
{
    ...
    if($approved == 1){
        if($commentdata['comment_post_ID']){
            ...
            insert_notification(null, $commentdata['user_id'], $commentdata['comment_post_ID']);
        }
    }
...
function userUpdateQuestion()
{
    ...
    if(!empty($errors)){
        ...
        if(!empty($inputCat)){
            wp_set_object_terms( $post_id, array_map('intval', explode(",", $inputCat)), 'question_category' );
        }
        ...
        if(!empty($inputImages)){
            ...
            $messenger = __('Images Missing', 'opalbeauty');
        }
...
function userCreateQuestion()
{
    ...
    $input = [
        ...
        'question_category' =>  isset($_POST['question_category']) ? $_POST['question_category'] : '',
    ...
    if(!empty($errors)){
        ...
        // Insert the post into the database
        $post_id = wp_insert_post( $my_post );
        wp_set_object_terms( $post_id, array_map('intval', explode(",", $input['question_category'])), 'question_category' );
        $attachment_ids = my_assets_uploader($_FILES[ 'question_image' ], $post_id);
...
function opal_check_user_doctor_updated( $user_id, $old_user_data ) {
    ...
    if($user->roles && $user->roles[0] && $user->roles[0] == 'doctor'){
        ...
        if($user_doctor){
            ...
            insert_notification_doctor(arr_notification()['refuse_doctor'], $user_id);
        }
    }
}
...
function handling_upload_image($user_id, $username, $field){
    try {
        ...
        //  WP_Filesystem();
        //  $bits = $_FILES['image']['tmp_name'];
        //  $loc = sanitize_title("user-". $username . '-' .rand_string(10)).".png";    
        
        //  $content_directory = $wp_filesystem->wp_content_dir() . 'uploads/';
        //  if ( ! is_dir( $content_directory . 'user-upload' ) ) {
        //     $wp_filesystem->mkdir( $content_directory . 'user-upload' );
        // }
      
        //  $target_dir_location = $content_directory . 'user-upload/';
         
        //  $imageUpload = move_uploaded_file( $bits, $target_dir_location . $loc );
 
        //  if ( $imageUpload ) {
        //     update_field( 'user_avatar',   $target_dir_location.$loc, $user_id );
        //  } else {
        //     return __( 'Unable to upload please try again', 'opalbeauty');
        //  }
        $attachment_id = '';
        ...
        update_field( $field[1],   $attach_id, $user_id );
    } catch (\Throwable $th) {
        return __( 'Unable to upload please try again', 'opalbeauty');
    }
}
...
function check_user_login(){
    ...
    }else if(is_page_template( 'page-login.php' ) || is_page_template( 'page-welcome.php' )){
        logged_in_redirect('');
    }else if(!is_admin()){
...
function get_notification(){
    ...
    WHERE n.user_id=%d 
    or n.notification_text IN ('added_event', 'posted_review', 'posted_question', 'update_new_spa')";
...
function hanlding_notification_posted_review($key, $value){
    ...
    return [
        'image' => $img ? $img : get_theme_path("/assets/images/user-icon.svg"),
        'title' => get_userdata( $value->user_id )->display_name. ' '. arr_notification()[$key][1],
        'link' => get_the_permalink($value->post_id),
